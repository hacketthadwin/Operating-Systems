...
//see there are two ways to include element in the cache
//our cache memory is limited, so we need to remove an element to insert a new one
//there are two ways to do this:
// 1. LRU (Least Recently Used) - remove the least recently used element
// 2. LFU (Least Frequently Used) - remove the least frequently used element
//we will use LRU for simplicity
//for LRU, we need to keep track of the last access time of each element
...
//different sockets for different clients
//all sockets will be connected to proxy server
//number of clients connected = number of threads created
//every thread will be connected to one socket
int port_number = 8080;				// Default Port
//jitne sockets open krenge utne alag alag threads me jayenge
...
//this is simply like a lock 
//consider there are 10 clients which are connected to the proxy server
//now when the 11th client tries to connect, the semaphore will call sem_wait()
//and it will block the 11th client until one of the previous clients disconnects
//when a client disconnects, the semaphore will call sem_post()
// and it will allow the 11th client to connect
//this is used to limit the number of clients that can connect to the proxy server at a given time

//sem_t cache_lock;			       
pthread_mutex_t lock;               //lock is used for locking the cache
//so consider both google.com and facebook.com are requested by 2 different clients
//they both will try to access the cache at the same time
//so when a shared resource is accessed by multiple threads, this is race condition
//to avoid this, we will use lock
